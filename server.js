...
if (TICKERALL_API_KEY && TICKERALL_API_KEY !== '') {
    try {
        ...
        console.log('📊 API Key: ' + TICKERALL_API_KEY.substring(0, 20) + '...');
    } catch (error) {
        console.error('❌ TickerAll initialization error:', error.message);
    }
} else {
    console.log('⚠️ TICKERALL_API_KEY not set. Please add your API key.');
}
...
// ==================== EXNESS ACCOUNT ROUTES (via TickerAll) ====================
app.post('/api/set-exness-creds', authenticate, async (req, res) => {
    try {
        ...
        if (!ticker) {
            return res.status(500).json({ success: false, message: 'TickerAll not configured. Please check API key.' });
        }
...
app.post('/api/connect-exness', authenticate, async (req, res) => {
    try {
        ...
        if (!ticker) {
            return res.status(500).json({ success: false, message: 'TickerAll not configured.' });
        }
...
async function getAISignal(symbol, accountId) {
    try {
        // Get historical rates from TickerAll
...
async function shouldClosePosition(position, accountId) {
    try {
        ...
        // Get current market conditions for dynamic decision
        const rates = await ticker.market.getHistory(accountId, {
...
app.post('/api/start-trading', authenticate, async (req, res) => {
    try {
        ...
        res.json({ 
            success: true, 
            sessionId, 
            message: `✅ HALAL TRADING STARTED! Investment: $${investmentAmount} | Target: $${targetProfit} | AI analyzes continuously and closes positions at maximum profit.` 
        });
    } catch (error) {
